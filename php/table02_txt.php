...
if ($clearTable) {
    $sqlClear = "TRUNCATE TABLE table02";
...
if (($handle = fopen($txtfile, "r")) !== FALSE) {
    ...
    $row = 0;
    while (($line = fgets($handle)) !== FALSE && $row < 85) {
        // Удаляем символы \r\n из строки
        $line = str_replace(["\r", "\n"], '', $line);
        ...
        $part2 = $conn->real_escape_string(mb_substr($line, 10, 25, 'UTF-8'));  // Следующие 15 символов
        $part3 = $conn->real_escape_string(mb_substr($line, 35, 180, 'UTF-8'));  // Последующие символы
        ...
        $sql = "INSERT INTO table02 (kod, tel1, klient) VALUES ('$part1', '$part2', '$part3')";
